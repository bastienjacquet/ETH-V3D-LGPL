...
void tv_flow_cuda(int w, int h, int nIterations, float alpha, float const theta, float tau, void * buffers_);
...
int main(int argc, char * argv[])
{
   if (argc != 6)
   {
      cerr << "Usage: " << argv[0] << " <left image> <right image> <nIterations> <alpha> <tau>" << endl;
      return -1;
   }
   ...
   for (int k = 0; k < 1; ++k)
   {
      ...
      for (int l = nLevels-1; l >= 0; --l)
      {
         ...
         //int const nLevelIterations = nIterations * (1+l);

         //float const thetaLevel = theta;
         float const thetaLevel = theta / float(1 << l);
         ...
         tv_flow_cuda(W, H, nLevelIterations, alphaLevel, thetaLevel, tau, cudaBuffers[l]);
