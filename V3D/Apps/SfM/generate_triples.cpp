...
/**
  * This program keeps pairwise correspondences that belong to geometricaly consistent triplets
  * @param <image list file>
  * @param <file.conf>
  * @param <radius>
  * @return it creates "triplets.db", "allcorrs.bin"
  *
  * Relevant <file.conf> options are MAX_REPROJECTION_ERROR_TRIPLET, MINIMUM_CORRESPONDENCES_TRIPLET
  * REQUIRED_TRIPLET_POINTS ,MINIMUM_TRIPLET_POINT_RATIO, MIN_TRIANGULATION_ANGLE
  *
  * It needs "calibration_db.txt", "pairwise_matches.db"
  */
int
main(int argc, char * argv[])
{
   ...
   try
   {
      ...
      for (map<ViewPair, int>::const_iterator pair01 = viewPairOIDMap.begin(); pair01 != viewPairOIDMap.end(); ++pair01)
      {
         int const view0    = (*pair01).first.view0;
         int const view1    = (*pair01).first.view1;
         int const oidPair01 = (*pair01).second;

         PairwiseMatch * match01Ptr = matchDataCache[oidPair01];
         ...
         for (int view2 = view1+1; view2 <= lastView; ++view2)
         {
            //cout << "(" << view0 << ", " << view1 << ", " << view2 << ")" << endl;

            Vector3d T01 = match01.translation;

            map<ViewPair, int>::const_iterator p02 = viewPairOIDMap.find(ViewPair(view0, view2));
            if (p02 == viewPairOIDMap.end()) continue;

            map<ViewPair, int>::const_iterator p12 = viewPairOIDMap.find(ViewPair(view1, view2));
            if (p12 == viewPairOIDMap.end()) continue;

            int const oidPair02 = (*p02).second;
            int const oidPair12 = (*p12).second;

            PairwiseMatch * match02Ptr = matchDataCache[oidPair02];
            if (match02Ptr->corrs.size() < nMinCorrs) continue;

            PairwiseMatch * match12Ptr = matchDataCache[oidPair12];
            ...
            filterTripleModel(calibDb, view0, view1, view2, maxReproError, tripleCams, tripleModel);
#if defined(PRINT_DEBUG_OUTPUT)
            cout << " after filtering with maxReproError = "<< maxReproError<<" tripleModel.size() = " << tripleModel.size() << endl;
#endif
            ...
            reconstruction.views[0] = view0;
            reconstruction.views[1] = view1;
            reconstruction.views[2] = view2;
            ...
            listItem.views = ViewTripletKey(view0, view1, view2);
            ...
            goodViewPairs.insert(ViewPair(view0, view1));
            goodViewPairs.insert(ViewPair(view0, view2));
            goodViewPairs.insert(ViewPair(view1, view2));
