#ifndef RECONSTRUCTION_COMMON_H
#define RECONSTRUCTION_COMMON_H
// -*- C++ -*-
...
//       ViewTripleKey(int v0, int v1, int v2)
//       {
//          //V3D::sort3(v0, v1, v2);
//          if (v0 > v1 || v1 > v2 || v0 > v2)
//             std::cerr << "ViewTripleKey::ViewTripleKey(): v0 < v1 < v2 failed! "
//                       << "This will break the SfM methods." << std::endl;
...
//       bool operator<(ViewTripleKey const& b) const
//       {
...
//       V3D_DEFINE_LOAD_SAVE(ViewTripleKey)

//       int views[3];
// };

// V3D_DEFINE_IOSTREAM_OPS(ViewTripleKey)
...
struct TripletAlignment
{
      ...
      void serialize(Archive& ar)
      {
         V3D::SerializationScope<Archive> scope(ar, "TripletAlignment");
         ar & oids[0] & oids[1];
         ar & weight & scale;
         ar & R;
         ar & T;
      ...
      V3D_DEFINE_LOAD_SAVE(TripletAlignment)
}; // end struct TripletAlignment
...
namespace V3D
{
   ...
   inline int ransacNSamples(int nNeededSamples, double outlierRatio, double wantedPsucces)
   {
      double const maxN = 1e8;
      if (outlierRatio >= 1.0) return int(maxN);

      double N = log(1.0 - wantedPsucces) / log(1.0 - pow((1.0 - outlierRatio), nNeededSamples));
   ...
   computeRobustAbsolutePoseMLE(std::vector<Vector2f> const& normPoints2d, std::vector<Vector3d> const& points3d,
                                ...
                                RobustOrientationResult& res, RobustOrientationMode mode = RobustOrientationMode()) throw();


   void adjustHomography(Matrix3x3d& H, vector<PointCorrespondence> const& corrs, int nIterations = 100);
   ...
   double
   computeRobustEuclideanTransformationMLE(std::vector<Vector3d> const& left, std::vector<Vector3d> const& right,
...
void writeMatchesToVRML(std::vector<T> const& points, U const & color, char const * filename, bool append = false)
{
    using namespace std;
    std::ofstream os;
...
inline void
writeGoodPointsToVRML(std::vector<V3D::CameraMatrix> const& cameras,
                      std::vector<V3D::TriangulatedPoint> const& model, char const * wrlName,
                      double inlierThreshold = 10.0, int minViews = 3)
{
   using namespace std;
   ...
   for (int j = 0; j < model.size(); ++j)
   {
      ...
      for (int k = 0; k < X.measurements.size(); ++k)
      {
         ...
         double err = distance_L2(p, X.measurements[k].pos);
         if (err > inlierThreshold)
         {
            isGood = false;
            continue;
...
inline void
writeQuantilePointsToVRML(std::vector<T> const &goodXs, char const * wrlName, float quantile, U const & color, bool append=false)
{
   using namespace std;
   using namespace V3D;
