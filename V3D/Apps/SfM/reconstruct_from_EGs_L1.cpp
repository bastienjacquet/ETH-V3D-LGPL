...
main(int argc, char * argv[])
{
   ...
   try
   {
      ...
      double const bundleInlierThreshold = 3.0 / 1000.0;
...
#else
            vector<double> zs;
            //V3D::computeConsistentRotations_Linf(0.1, 10000, nCompViews, tmpRelRotations, viewPairVec, rotations, zs);
            V3D::computeConsistentRotations_L1(0.0001, 10000, nCompViews, tmpRelRotations, viewPairVec, rotations);
            //V3D::computeConsistentRotations_LSQ(10000, nCompViews, tmpRelRotations, viewPairVec, rotations);
...
# if 0
               V3D::StdMetricBundleOptimizer opt(bundleInlierThreshold, savedCameras, savedPoints, bundleStruct.measurements,
...
# else
               ...
               StdDistortionFunction distortion;
               V3D::CommonInternalsMetricBundleOptimizer opt(bundleMode, bundleInlierThreshold, K, distortion,
                                                             savedCameras, savedPoints, bundleStruct.measurements,
                                                             bundleStruct.correspondingView, bundleStruct.correspondingPoint);
# endif
               opt.tau = 1e-3;
...
#else
               V3D::StdMetricBundleOptimizer opt(bundleInlierThreshold, cameras, bundleStruct.points3d, bundleStruct.measurements,
                                                 bundleStruct.correspondingView, bundleStruct.correspondingPoint);
               V3D::optimizerVerbosenessLevel = 1;
               opt.tau = 1e-3;
