...
main(int argc, char * argv[])
{
   ...
   {
      cerr << "Usage: " << argv[0] << " <config file> [<submodel alignment blacklist file> [<membership blacklist file>]]" << endl;
      return -1;
   }
   ...
   try
   {
      ...
      bool   const useMST_Upgrade = cf.get("USE_MST_UPGRADE", true);
...
#else
            Matrix<double> B(nPairs, nSubModels, 0.0);
            for (int k = 0; k < nPairs; ++k)
            {
               int const i0 = subModelPairs[k].first;
               int const i1 = subModelPairs[k].second;

               double const weight = relWeights[k];

               B[k][i0] = weight;
               B[k][i1] = -weight*relScales[k];
            } // end for (k)
            cout << "B = "; displayMatrix(B);
            SVD<double> svd(B);
            Matrix<double> const& V = svd.getV();
            V.getColumnSlice(0, V.num_rows(), V.num_cols()-1, scales);
#endif
...

         {
            ...
            for (size_t i = 0; i < finalReconstruction.size(); ++i)
            {
               ...
               for (int k = 0; k < X.measurements.size(); ++k)
               {
                  ...
                  m.view = compViewIdBackMap[m.view];
