...
using namespace V3D;
/**
  * This program generates a text file database (named "calibration_db.txt") with the intrinsic parameter for each image
  * It uses, by priority a per image .calib file; per image EXIF informations; global config file parameters
  *
  * calib file : fx skew cx fy cy
  *              k1 k2 p1 p2
            ...
            {
               {
                  int w, h;
                  double fx, fy;
                  readEXIF = getCalibrationFromEXIF(entries[i].c_str(), w, h, fx, fy, -1, -1);
                  cout << entries[i].c_str() << ": w = " << w << " h = " << h << " fx = " << fx << endl;
               }
               int w, h;
               double fx, fy;
