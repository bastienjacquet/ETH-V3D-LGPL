...
/**
  * This program generate multiple 3D submodel
  * @param <image list file>
  * @param <file.conf>
  * @param <radius>
  * it creates "pairwise_matches.db", "triplets.db", "submodels.db"
  * and (only for information) "matchedPairs.txt" "panoramaPairs.txt"
  * Relevant <file.conf> options are MAX_REPROJECTION_ERROR_SUBMODEL (reprojection error is w.r.t. 1024xX pixel image), MAX_REPROJECTION_ERROR_TRIPLET,
  * REQUIRED_TRIPLE_POINTS_SUBMODEL, REQUIRED_TRIPLET_POINTS, REQUIRED_VISIBLE_POINTS, SUBMODEL_VIEW_FREQUENCY,
  * SUBMODEL_MAX_SIZE, SUBMODEL_BUNDLE_ITERATIONS, MIN_FILTERED_POINT_COUNT_RATIO, USE_RANDOM_SUBMODEL_GROWING
...
//             }
      ...
      for (size_t subModelId = 0; subModelId < submodelsViews.size(); ++subModelId)
      {
         ...
         cout << " nViews = " << subModel._nViews << ", nPairs = " << subModel._viewPairs.size()
              << ", nTriplets = " << subModel._triplets.size() << endl;

         subModel.computeConsistentRotations(relRotations);

         bool const reestimateScaleRations = false;
         subModel.computeConsistentTranslationLengths(relTranslations, matchDataCache, viewPairOIDMap,
                                                      tripletDataCache, tripletOIDMap, reestimateScaleRations);
         subModel.computeConsistentTranslations();
