...
namespace V3D
{
                                           ...
                                           Matrix3x3d& R, Vector3d& T, std::vector<int> &inliers)
   {
       ...
       double score = sampleEuclideanTransform(left, right, &indices[0], inlierThresholdL, inlierThresholdR,
                                               R, T, inliers);
       inlierRatio = float(inliers.size()) / left.size();
       expectedRuns = ransacNSamples<3>(inlierRatio, confidence);
       ...
       while (1)
       {
           ...
           Matrix3x3d R1;
           Vector3d T1;
           double score1 = sampleEuclideanTransform(left, right, &indices[sampleOffset],
                                                    inlierThresholdL, inlierThresholdR,
                                                    R1, T1, curInliers);
       ...
       return double(inliers.size()) / left.size();
   } // end computeRobustEuclideanTransformationMLE()
