...
namespace V3D
{
   ...
   inline Matrix3x3d
   getRelativeRotation(Matrix3x3d const& R1, Matrix3x3d const& R2)
   {
      return R2 * R1.transposed();
   }
   ...
   inline void
   getRelativeOrientation(Matrix3x4d const& RT1, Matrix3x4d const& RT2, Matrix3x3d& R_rel, Vector3d& T_rel)
   {
      Matrix3x3d R1, R2;
      Vector3d   T1, T2;
      copyMatrixSlice(RT1, 0, 0, 3, 3, R1, 0, 0);
...
//**********************************************************************
   ...
   void computeFundamentalsFrom7Corrs(double const x1[7], double const y1[7], double const x2[7], double const y2[7],
