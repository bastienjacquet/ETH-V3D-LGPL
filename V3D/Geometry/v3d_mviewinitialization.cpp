...
//    double
//    computeCheiralityProximityLipschitzConstant(std::vector<Matrix3x3d> const& rotations,
//                                                std::vector<TriangulatedPoint>& sparseModel)
//    {
//       int const N = rotations.size();
//       int const M = sparseModel.size();
...
//       // We have K dual variables and N + 3*M + K primal ones
...
//          int const Ti2 = i;
//          int const Xj0 = N + 3*j + 0;
//          int const Xj1 = N + 3*j + 1;
//          int const Xj2 = N + 3*j + 2;
//          int const Dij = N + 3*M + k;

//          int const mu  = k;

//          nz.push_back(make_pair(mu, Xj0)); values.push_back(-R[2][0]);
//          nz.push_back(make_pair(mu, Xj1)); values.push_back(-R[2][1]);
//          nz.push_back(make_pair(mu, Xj2)); values.push_back(-R[2][2]);
//          nz.push_back(make_pair(mu, Ti2)); values.push_back(-1.0);
//          nz.push_back(make_pair(mu, Dij)); values.push_back(-1.0);
//       } // end for (k)

//       CCS_Matrix<double> A(K, N + 3*M + K, nz, values);
...
//       bool status = computeSparseSVD(A, V3D_ARPACK_LARGEST_MAGNITUDE_EIGENVALUES, 1, sv, V);
//       return sv[0];
//    } // end computeCheiralityProximityLipschitzConstant()

//    void
//    computeProximalCheirality(std::vector<Matrix3x3d> const& rotations,
//                              std::vector<Vector3d>& translations,
//                              std::vector<Vector3d>& Xs, std::vector<double>& Ds,
//                              vector<int> const& correspondingView, vector<int> const& correspondingPoint,
//                              int const nIterations, double const tau_p, double const tau_d)
//    {
//       // 0 is the hybrid scheme, 1 is Chambolle's method
// #define NUMERICAL_SCHEME 1

//       float const maxMu = 100.0;
...
//       vector<double> Ts(N);
//       for (int i = 0; i < N; ++i) Ts[i] = translations[i][2];
...
//          double   const  T3 = Ts[i];
//          double   const XX3 = R[2][0]*X[0] + R[2][1]*X[1] + R[2][2]*X[2] + T3;
//          if (XX3 < 1.0) Ds[k] = 1.0 - XX3;
//       }

//       vector<double> const Ts_orig(Ts);
//       vector<double> const Ds_orig(Ds);
//       vector<Vector3d> const Xs_orig(Xs);

//       vector<float> mu(K, 0.0f);
//       vector<double> dDs_k(K);

// #if NUMERICAL_SCHEME == 1
//       vector<Vector3d> XXs(Xs);
//       vector<double> TTs(Ts);
//       vector<double> DDs(Ds);
// #endif

//       vector<double> dTs(N, 0.0);
...
//             dXs[j][0] += (X[0] - Xs_orig[j][0]) - mu[k]*R[2][0];
//             dXs[j][1] += (X[1] - Xs_orig[j][1]) - mu[k]*R[2][1];
//             dXs[j][2] += (X[2] - Xs_orig[j][2]) - mu[k]*R[2][2];
//             dTs[i]    += (Ts[i] - Ts_orig[i]) - mu[k];
//             dDs_k[k]   = (Ds[k] - Ds_orig[k]) - mu[k];
//          } // end for (k)

//          //cout << "dTs = "; displayVector(dTs);

// #if NUMERICAL_SCHEME == 1
//          // Chambolle
//          for (int i = 1; i < N; ++i) TTs[i] -= 2*tau_p * dTs[i];
//          for (int i = 1; i < N; ++i) Ts[i]  -= tau_p * dTs[i];

//          for (int j = 0; j < M; ++j) addVectors(Xs[j], -2*tau_p * dXs[j], XXs[j]);
//          for (int j = 0; j < M; ++j) addVectorsIP(-tau_p * dXs[j], Xs[j]);

//          for (int k = 0; k < K; ++k)
//          {
//             double const d_new = std::max(0.0, Ds[k] - tau_p * dDs_k[k]);
//             DDs[k] = 2*d_new - Ds[k];
//             Ds[k]  = d_new;
//          }
// #else
...
//             double const XX3 = R[2][0]*X[0] + R[2][1]*X[1] + R[2][2]*X[2] + T3 + D;

//             mu[k] += tau_d * (1.0 - XX3);
//             mu[k] = clamp(mu[k], 0.0f, maxMu);
//          } // end for (k)
//       } // end for (iter)

//       for (int i = 1; i < N; ++i) translations[i][2] = Ts[i];

// //       cout << "Ts_orig = "; displayVector(Ts_orig);
// //       cout << "Ts = "; displayVector(Ts);
// //       cout << "Ds_orig = "; displayVector(Ds_orig);
// //       cout << "Ds = "; displayVector(Ds);
// //       cout << "mu = "; displayVector(mu);
...
//             Matrix3x3d const& R = rotations[i];
//             Vector3d   const& X = Xs[j];
//             double const XX3 = R[2][0]*X[0] + R[2][1]*X[1] + R[2][2]*X[2] + Ts[i] + Ds[k];
//             if (XX3 >= 1.0) ++nFeasible;
//          }
//          cout << "computeProximalCheirality(): " << nFeasible << "/" << K << " measurements satisfy cheirality." << endl;
//       }
// #undef NUMERICAL_SCHEME
//    } // end computeProximalCheirality()
...
//    void
//    computeConsistentTranslationsConic_L1_PD(float const sigma,
//                                             std::vector<Matrix3x3d> const& rotations,
//                                             std::vector<Vector3d>& translations,
//                                             std::vector<TriangulatedPoint>& sparseModel,
//                                             TranslationRegistrationPD_Params const& params)
//    {
//       // 0 is the hybrid scheme, 1 is Chambolle's method, 2 is Popov's approach.
//       // 3 is a hybrid P-D scheme alternating between Xj and Ti
// #define NUMERICAL_SCHEME 1
...
//       vector<Vector4f> Ys(K);
//       vector<float>    Zs(K, 0.0);

//       for (int k = 0; k < K; ++k) makeZeroVector(Ys[k]);
//       for (int i = 0; i < N; ++i) makeZeroVector(translations[i]);
//       for (int j = 0; j < M; ++j) makeZeroVector(Xs[j]);

// #if NUMERICAL_SCHEME == 1 || NUMERICAL_SCHEME == 4
...
//       vector<Vector4f> YYs(K);
//       vector<float>    ZZs(K, 0.0);
//       for (int k = 0; k < K; ++k) makeZeroVector(YYs[k]);
//       vector<Vector4f> dYs(K);
//       vector<float>    dZs(K, 0.0);
// #endif
...
//       {
// #if NUMERICAL_SCHEME == 0 || NUMERICAL_SCHEME == 1
//          // Primal updates
//          for (int k = 0; k < K; ++k)
//          {
//             int const i = correspondingView[k];
...
//             float const y1 = Ys[k][0], y2 = Ys[k][1];
//             float const y3 = Ys[k][2], y4 = Ys[k][3];

//             float const Yx = y1 - y2; // combined dual variable
//             float const Yy = y3 - y4;
//             float const YYx = y1 + y2; // absolute value of dual variable
//             float const YYy = y3 + y4;
...
//             dTs_cur[0] = -Yx;
//             dTs_cur[1] = -Yy;
//             dTs_cur[2] = Yx*u1 + Yy*u2 - sigma*(YYx + YYy);

//             dXs_cur[0] = Yx * (u1*R[2][0] - R[0][0]) - sigma * YYx * R[2][0];
//             dXs_cur[1] = Yx * (u1*R[2][1] - R[0][1]) - sigma * YYx * R[2][1];
//             dXs_cur[2] = Yx * (u1*R[2][2] - R[0][2]) - sigma * YYx * R[2][2];

//             dXs_cur[0] += Yy * (u2*R[2][0] - R[1][0]) - sigma * YYy * R[2][0];
//             dXs_cur[1] += Yy * (u2*R[2][1] - R[1][1]) - sigma * YYy * R[2][1];
//             dXs_cur[2] += Yy * (u2*R[2][2] - R[1][2]) - sigma * YYy * R[2][2];

//             // Add barriers:
//             dTs_cur[2] += -Zs[k];
//             dXs_cur[0] += -Zs[k] * R[2][0];
//             dXs_cur[1] += -Zs[k] * R[2][1];
//             dXs_cur[2] += -Zs[k] * R[2][2];
...
// #if NUMERICAL_SCHEME == 1
//          // Chambolle
//          for (int i = 1; i < N; ++i) addVectors(translations[i], -2*tau_p * dTs[i], TTs[i]);
...
//             double const XX3 = R[2][0]*X[0] + R[2][1]*X[1] + R[2][2]*X[2] + T[2];
//             Ys[k][0] += tau_d * ( u1*XX3 - XX1 - sigma*XX3);
//             Ys[k][1] += tau_d * (-u1*XX3 + XX1 - sigma*XX3);
//             Ys[k][2] += tau_d * ( u2*XX3 - XX2 - sigma*XX3);
//             Ys[k][3] += tau_d * (-u2*XX3 + XX2 - sigma*XX3);
//             Zs[k] += -tau_d * (XX3 - 1.0);

//             Ys[k][0] = clamp(Ys[k][0], 0.0f, 1.0f);
//             Ys[k][1] = clamp(Ys[k][1], 0.0f, 1.0f);
//             Ys[k][2] = clamp(Ys[k][2], 0.0f, 1.0f);
//             Ys[k][3] = clamp(Ys[k][3], 0.0f, 1.0f);
//             Zs[k] = clamp(Zs[k], 0.0f, maxZ);

// //             float const YYx = Ys[k][0] + Ys[k][1];
// //             float const YYy = Ys[k][2] + Ys[k][3];
// //             if (YYx > 1.0f)
// //             {
// //                Ys[k][0] -= (YYx - 1.0) / 2;
// //                Ys[k][1] -= (YYx - 1.0) / 2;
...
//             dYs[k][0] =  u1*XX3 - XX1 - sigma*XX3;
//             dYs[k][1] = -u1*XX3 + XX1 - sigma*XX3;
//             dYs[k][2] =  u2*XX3 - XX2 - sigma*XX3;
//             dYs[k][3] = -u2*XX3 + XX2 - sigma*XX3;
//             dZs[k]    = 1.0 - XX3;
//          } // end for (k)
...
//          // Keep T[0] fixed to origin
//          // Primal updates
//          for (int i = 1; i < N; ++i) addVectorsIP(-tau_p * dTs[i], TTs[i]);
//          for (int i = 1; i < N; ++i) addVectors(TTs[i], -tau_p * dTs[i], translations[i]);

//          for (int j = 0; j < M; ++j) addVectorsIP(-tau_p * dXs[j], XXs[j]);
//          for (int j = 0; j < M; ++j) addVectors(XXs[j], -tau_p * dXs[j], Xs[j]);

//          // Dual updates
//          for (int k = 0; k < K; ++k)
//          {
//             YYs[k][0] += tau_d * dYs[k][0]; YYs[k][0] = clamp(YYs[k][0], 0.0f, 1.0f);
//             YYs[k][1] += tau_d * dYs[k][1]; YYs[k][1] = clamp(YYs[k][1], 0.0f, 1.0f);
//             YYs[k][2] += tau_d * dYs[k][2]; YYs[k][2] = clamp(YYs[k][2], 0.0f, 1.0f);
//             YYs[k][3] += tau_d * dYs[k][3]; YYs[k][3] = clamp(YYs[k][3], 0.0f, 1.0f);
//             ZZs[k]    += tau_d * dZs[k];    ZZs[k] = clamp(ZZs[k], 0.0f, maxZ);

//             Ys[k][0] = YYs[k][0] + tau_d * dYs[k][0]; Ys[k][0] = clamp(Ys[k][0], 0.0f, 1.0f);
//             Ys[k][1] = YYs[k][1] + tau_d * dYs[k][1]; Ys[k][1] = clamp(Ys[k][1], 0.0f, 1.0f);
//             Ys[k][2] = YYs[k][2] + tau_d * dYs[k][2]; Ys[k][2] = clamp(Ys[k][2], 0.0f, 1.0f);
//             Ys[k][3] = YYs[k][3] + tau_d * dYs[k][3]; Ys[k][3] = clamp(Ys[k][3], 0.0f, 1.0f);
//             Zs[k]    = ZZs[k]    + tau_d * dZs[k];    Zs[k] = clamp(Zs[k], 0.0f, maxZ);
//          } // end for (k)
// #elif NUMERICAL_SCHEME == 3 || NUMERICAL_SCHEME == 4
//          // Primal updates, update cameras first
//          for (int k = 0; k < K; ++k)
//          {
//             int const i = correspondingView[k];
//             int const j = correspondingPoint[k];
...
//             Ys[k][0] += tau_d * ( u1*XX3 - XX1 - sigma*XX3); Ys[k][0] = clamp(Ys[k][0], 0.0f, 1.0f);
//             Ys[k][1] += tau_d * (-u1*XX3 + XX1 - sigma*XX3); Ys[k][1] = clamp(Ys[k][1], 0.0f, 1.0f);
//             Ys[k][2] += tau_d * ( u2*XX3 - XX2 - sigma*XX3); Ys[k][2] = clamp(Ys[k][2], 0.0f, 1.0f);
//             Ys[k][3] += tau_d * (-u2*XX3 + XX2 - sigma*XX3); Ys[k][3] = clamp(Ys[k][3], 0.0f, 1.0f);
//             Zs[k]    += -tau_d * (XX3 - 1.0);                Zs[k] = clamp(Zs[k], 0.0f, maxZ);
//          } // end for (k)

//          // Second pass, compute structure derivatives
//          for (int k = 0; k < K; ++k)
//          {
//             int const i = correspondingView[k];
//             int const j = correspondingPoint[k];
...
//                E += std::max(0.0, fabs(XX1 - u1*XX3) - c);
//                E += std::max(0.0, fabs(XX2 - u2*XX3) - c);
//                E += maxZ * std::max(1.0 - XX3, 0.0);
//             } // end for (k)
//             cout << "E = " << E << endl;
...
//                Matrix3x3d const& R = rotations[i];
//                Vector3d   const& X = Xs[j];
//                double const XX3 = R[2][0]*X[0] + R[2][1]*X[1] + R[2][2]*X[2] + translations[i][2];
//                if (XX3 >= 1.0) ++nFeasible;
//             }
...
//             nFeasible = 0;
//             for (int k = 0; k < K; ++k)
//                if (Ys[k][0] + Ys[k][1] <= 1.0 &&
//                    Ys[k][2] + Ys[k][3] <= 1.0) ++nFeasible;
//             cout << nFeasible << "/" << K << " measurements satisfy Y1+Y2 <= 1." << endl;
...
//       //cout << "Zs = "; displayVector(Zs);
//       {
//          int nnz = 0;
//          for (int k = 0; k < K; ++k) if (Zs[k] > 0.0) ++nnz;
//          cout << nnz << " non-zeros in Zs" << endl;
//       }

// //       {
// //          vector<float> YYx(K), YYy(K);
// //          for (int k = 0; k < K; ++k)
// //          {
// //             YYx[k] = Ys[k][0] + Ys[k][1];
// //             YYy[k] = Ys[k][2] + Ys[k][3];
// //          }
// //          cout << "YYx = "; displayVector(YYx);
// //          cout << "YYy = "; displayVector(YYy);
// //       }
...
//       }
// #undef NUMERICAL_SCHEME
//    } // end computeConsistentTranslationsConic_L1_PD()
...
//    void
//    computeConsistentTranslationsConic_L1_PD2(float const sigma,
//                                              std::vector<Matrix3x3d> const& rotations,
//                                              std::vector<Vector3d>& translations,
//                                              std::vector<TriangulatedPoint>& sparseModel,
//                                              TranslationRegistrationPD_Params const& params)
//    {
//       typedef InlineVector<float, 6>  Vector6f;
...
//             float const z1x     = Zs[k][0] - Zs[k][1];
//             //float const z1x_mag = Zs[k][0] + Zs[k][1];
//             float const z2x     = Zs[k][2];

//             float const z1y     = Zs[k][3] - Zs[k][4];
//             //float const z1y_mag = Zs[k][3] + Zs[k][4];
//             float const z2y     = Zs[k][5];

//             Vector3d dTs_cur, dXs_cur;

//             dTs_cur[0] = -z1x;
//             dTs_cur[1] = -z1y;
//             dTs_cur[2] = z1x*u1 + z1y*u2 + sigmaZ2*(z2x + z2y);

//             dXs_cur[0] = z1x * (u1*R[2][0] - R[0][0]) + sigmaZ2 * z2x * R[2][0];
//             dXs_cur[1] = z1x * (u1*R[2][1] - R[0][1]) + sigmaZ2 * z2x * R[2][1];
//             dXs_cur[2] = z1x * (u1*R[2][2] - R[0][2]) + sigmaZ2 * z2x * R[2][2];

//             dXs_cur[0] += z1y * (u2*R[2][0] - R[1][0]) + sigmaZ2 * z2y * R[2][0];
//             dXs_cur[1] += z1y * (u2*R[2][1] - R[1][1]) + sigmaZ2 * z2y * R[2][1];
//             dXs_cur[2] += z1y * (u2*R[2][2] - R[1][2]) + sigmaZ2 * z2y * R[2][2];

//             dTs_k[k] = dTs_cur;
//             dXs_k[k] = dXs_cur;

//             dDs_k[k] = z1x*u1 + z1y*u2 + sigmaZ2*(z2x + z2y) + 1.0;
//          } // end for (k)
...
//             Zs[k][0] += tau_d * ( u1*XX3 - XX1 - sigma);
//             Zs[k][1] += tau_d * (-u1*XX3 + XX1 - sigma);
//             Zs[k][2] += tau_d * sigmaZ2 * (XX3 - 1.0);

//             Zs[k][3] += tau_d * ( u2*XX3 - XX2 - sigma);
//             Zs[k][4] += tau_d * (-u2*XX3 + XX2 - sigma);
//             Zs[k][5] += tau_d * sigmaZ2 * (XX3 - 1.0);

//             projectZs(Zs[k][0], Zs[k][1], Zs[k][2]);
//             projectZs(Zs[k][3], Zs[k][4], Zs[k][5]);
//          } // end for (k)
...
//             nFeasible = 0;
//             for (int k = 0; k < K; ++k)
//                if (Zs[k][0] + Zs[k][1] <= 1.0 &&
//                    Zs[k][3] + Zs[k][4] <= 1.0) ++nFeasible;
//             cout << nFeasible << "/" << K << " measurements satisfy Z^+ + Z^- <= 1." << endl;
...
//       }
// #undef NUMERICAL_SCHEME
//    } // end computeConsistentTranslationsConic_L1_PD2()

//    void
//    computeConsistentTranslationsConic_L1_PD4(float const sigma,
//                                              std::vector<Matrix3x3d> const& rotations,
//                                              std::vector<Vector3d>& translations,
//                                              std::vector<TriangulatedPoint>& sparseModel,
//                                              TranslationRegistrationPD_Params const& params)
...
//             dDs_k[k] = Yx*u1 + Yy*u2 - sigma*(YYx + YYy) + 1.0;
//          } // end for (k)
...
//       }
// #undef NUMERICAL_SCHEME
//    } // end computeConsistentTranslationsConic_L1_PD4()
...
//          L = sqr(computeTranslationRegistrationLipschitzConstant(sigma, rotations, sparseModel));
//          if (params.verbose) cout << "|A|^2 = " << L << endl;
// #else
//          cerr << "computeConsistentTranslationsConic_LP_BOS(): ARPACK not available for |A|_2 computation, assuming default value for L" << endl;
//          L = 1000.0;
// #endif
//       } // end if

//       double const delta = 0.95 / params.alpha / L;
...
//             dTs_cur[0] = -y1;
//             dTs_cur[1] = -y2;
//             dTs_cur[2] = y1*u1 + y2*u2 + sigma*(y3 + y4);

//             dXs_cur[0] = y1 * (u1*R[2][0] - R[0][0]) + sigma * y3 * R[2][0];
//             dXs_cur[1] = y1 * (u1*R[2][1] - R[0][1]) + sigma * y3 * R[2][1];
//             dXs_cur[2] = y1 * (u1*R[2][2] - R[0][2]) + sigma * y3 * R[2][2];

//             dXs_cur[0] += y2 * (u2*R[2][0] - R[1][0]) + sigma * y4 * R[2][0];
//             dXs_cur[1] += y2 * (u2*R[2][1] - R[1][1]) + sigma * y4 * R[2][1];
//             dXs_cur[2] += y2 * (u2*R[2][2] - R[1][2]) + sigma * y4 * R[2][2];
...
//          // Keep T[0] fixed to origin
//          for (int i = 1; i < N; ++i) addVectors(translations[i], -2*params.alpha * dTs[i], TTs[i]);
//          for (int i = 1; i < N; ++i) addVectorsIP(-params.alpha * dTs[i], translations[i]);

//          for (int j = 0; j < M; ++j) addVectors(Xs[j], -2*params.alpha * dXs[j], XXs[j]);
//          for (int j = 0; j < M; ++j) addVectorsIP(-params.alpha * dXs[j], Xs[j]);
...
//             float const yy0 = Ys[k][0] + delta * (u1*XX3 - XX1);
//             if (yy0 - delta*sigma > 0)
//             {
//                Ys[k][0] = yy0 - delta*sigma;
//             }
//             else if (yy0 + delta*sigma < 0)
//             {
//                Ys[k][0] = yy0 + delta*sigma;
//             }
//             else
...
//             float const yy1 = Ys[k][1] + delta * (u2*XX3 - XX2);
//             if (yy1 - delta*sigma > 0)
//             {
//                Ys[k][1] = yy1 - delta*sigma;
//             }
//             else if (yy1 + delta*sigma < 0)
//             {
//                Ys[k][1] = yy1 + delta*sigma;
//             }
//             else
...
//             Ys[k][2] += delta * sigma * (XX3 - 1.0);
//             Ys[k][3] += delta * sigma * (XX3 - 1.0);

// #if 0
//             Ys[k][0] = clamp(Ys[k][0], -1.0f, 1.0f);
//             Ys[k][1] = clamp(Ys[k][1], -1.0f, 1.0f);
//             Ys[k][2] = std::min(0.0f, Ys[k][2]);
//             Ys[k][3] = std::min(0.0f, Ys[k][3]);
// #else
//             projectZs(Ys[k][0], Ys[k][2]);
//             projectZs(Ys[k][1], Ys[k][3]);
// #endif
//          } // end for (k)

//          if (((iter+1) % params.reportFrequency) == 0 || iter == params.nIterations-1)
//          {
//             cout << "iter = " << iter << endl;
...
//          X.pos = Xs[j];
//       }
//    } // end computeConsistentTranslationsConic_L1_BOS()
