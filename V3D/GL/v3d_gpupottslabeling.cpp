...
//======================================================================
   ...
   void
   PottsLabeling3_Huber_FwBw::allocate(int w, int h)
   {
      _width = w;
      _height = h;
      ...
      _uBufA->allocate(w, h);
      _uBufB->allocate(w, h);
   } // end PottsLabeling3_Huber_FwBw::allocate()

   void
   PottsLabeling3_Huber_FwBw::deallocate()
   {
      _uBufA->deallocate();
      _uBufA->deallocate();
   } // end PottsLabeling3_Huber_FwBw::deallocate()

   void
   PottsLabeling3_Huber_FwBw::setZero()
   {
      glClearColor(1.0f/3, 1.0f/3, 1.0f/3, 0);
      glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT);
      _uBufA->activate();
      glClear(GL_COLOR_BUFFER_BIT);
   } // PottsLabeling3_Huber_FwBw::setZero()
   ...
   void
   PottsLabeling3_Huber_FwBw::copyBuffersFrom(PottsLabeling3_Huber_FwBw& src)
   {
      setupNormalizedProjection();
      _uBufA->activate();
      src._uBufA->enableTexture(GL_TEXTURE0);
      ...
      renderNormalizedQuad();
      src._uBufA->disableTexture(GL_TEXTURE0);
   } // end PottsLabeling3_Huber_FwBw::copyLabels()

   void
   PottsLabeling3_Huber_FwBw::iterate(unsigned int costTexId, int nIterations)
   {
      static Cg_FragmentProgram * uShader = 0;

      if (uShader == 0)
      {
         uShader = new Cg_FragmentProgram("PottsLabeling3_Huber_FwBw::uShader");
         uShader->setProgramFromFile("potts_3labeling_huber_fwbw_update_u.cg");
      ...
      uShader->parameter("tau", _tau * _epsilon);
      uShader->parameter("epsilon", _epsilon, 1.0f/_epsilon);
      ...
      glActiveTexture(GL_TEXTURE1);
      glDisable(GL_TEXTURE_2D);
   } // end PottsLabeling3_Huber_FwBw::iterate()
