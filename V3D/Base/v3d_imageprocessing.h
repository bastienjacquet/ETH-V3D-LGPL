...
//=========================================================================
   ...
   template <typename T>
   inline void
   resampleImageNearest(V3D::Image<T> const& src, V3D::Image<T>& dst, int const ofsx = 0, int const ofsy = 0)
   {
      assert(src.numChannels() == dst.numChannels());

      int const w = dst.width();
      int const h = dst.height();
      int const nChannels = dst.numChannels();

      int const W = src.width();
      int const H = src.height();

      float const wRatio = float(W) / w;
      float const hRatio = float(H) / h;

      for (int ch = 0; ch < nChannels; ++ch)
      {
         for (int y = 0; y < h; ++y)
         {
            int const ySrc = std::max(0, std::min(H-1, int(y*hRatio + 0.5f) + ofsy));
            for (int x = 0; x < w; ++x)
            {
               int const xSrc = std::max(0, std::min(W-1, int(x*wRatio + 0.5f) + ofsx));
               dst(x, y, ch) = src(xSrc, ySrc, ch);
            }
         } // end for (y)
      } // end for (ch)
   } // end resampleImageNearest()
...
//=========================================================================
   ...
   template<typename T>
   inline void
   boxFilterHorizontal(V3D::Image<T> const& src, int const kernelSize, int const center, T const& multiplier, V3D::Image<T>& dst)
   {
      int const w = src.width();
      int const h = src.height();
      int const nChannels = src.numChannels();
      ...
      for (int ch = 0; ch < nChannels; ++ch)
      {
         for (int y = 0; y < h; ++y)
         {
            for (int x = 0; x < w; ++x)
            {
               ...
               dst(x, y, ch) = sum * multiplier;
            } // end for (dx)
         } // end for (y)
      } // end for (ch)
   } // end boxFilterHorizontal()

   template<typename T>
   inline void
   boxFilterVertical(V3D::Image<T> const& src, int const kernelSize, int const center, T const& multiplier, V3D::Image<T>& dst)
   {
      int const w = src.width();
      int const h = src.height();
      int const nChannels = src.numChannels();
   ...
   } // end boxFilterVertical()

   inline void
   applySobelFilters(V3D::Image<float> const& src, V3D::Image<float>& Dx, V3D::Image<float>& Dy)
   {
      float const kernelG[] = { 1.0/4, 2.0/4, 1.0/4 };
      float const kernelD[] = { -1.0f, 0.0f, 1.0f };

      int const w = src.width();
      int const h = src.height();

      V3D::Image<float> tmp1(w, h, 1);
      convolveImageHorizontal(src, 3, 1, kernelD, tmp1);
      convolveImageVertical(tmp1, 3, 1, kernelG, Dx);

      convolveImageHorizontal(src, 3, 1, kernelG, tmp1);
      convolveImageVertical(tmp1, 3, 1, kernelD, Dy);
   } // end applySobelFilters()

   template<typename T, typename Op>
   inline void
   convolveImageHorizontal(V3D::Image<T> const& src, int const kernelSize, int const center, Op op, V3D::Image<T>& dst)
   {
      int const w = src.width();
      int const h = src.height();
      int const nChannels = src.numChannels();
   ...
   template<typename T, typename Op>
   inline void
   convolveImageVertical(V3D::Image<T> const& src, int const kernelSize, int const center, Op op, V3D::Image<T>& dst)
   {
      int const w = src.width();
      int const h = src.height();
      int const nChannels = src.numChannels();
