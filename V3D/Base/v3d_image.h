...
//----------------------------------------------------------------------
   ...
   void saveDataImageFile(const Image<Elem> &image, const char *filename)
   {
      ...
      verify(out.is_open(),"Failed to open image data file.");
      // Write image stat.
      ImageFileStat stat;
   ...
   template<typename Elem>
   void saveDataImageFileFromPtr(const Elem* ptr,int w,int h,int d, const char *filename)
   {
      // Open file.
      std::ofstream out(filename,std::ios_base::out|std::ios_base::binary);
      verify(out.is_open(),"Failed to open image data file.");
      ...
      out.write((char*)&type,sizeof(unsigned int));
      // Write image data.
      out.write((char*)ptr,sizeof(Elem)*stat.width*stat.height*stat.numChannels);
   }
